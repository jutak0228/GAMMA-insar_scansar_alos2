...
func_interferometry()
{
	...
	mas_to_slv="${master_dt}to${slave_dt}"
	...
	SLC_diff_intf $master_rslc $slave_rslc $master_rslc_par $slave_rslc_par $off_file $sim_unw_file $diff_file $rglks $azlks 1 0 0.2
...
do
	...
	do
		...
		if [ ${master_date} -lt ${slave_date} ];then
			...
			func_interferometry ${master} ${slave} ${rglks} ${azlks}
		fi
	done
done
